 ...
 *
 * Growth Stack UTM tagging for Pulse campaign → landing attribution.
 */
...
/** Hosts where Pulse reads utm_* on landing pageviews. */
...
export function isPulseTrackedLandingHost(hostname: string): boolean {
  const host = hostname.toLowerCase().replace(/^www\./, '');
  return PULSE_TRACKED_LANDING_HOSTS.some((tracked) => host === tracked || host.endsWith(`.${tracked}`));
}

/** Tag outbound campaign links to a public landing site (brand URL or known Pulse host). */
export function campaignLandingUrl(
  ...
  opts: {
    ...
    medium?: UtmMedium;
    content?: string;
    /** When false, only tag known Pulse landing hosts. Default true for campaign exports. */
    force?: boolean;
  },
): string {
  const raw = url?.trim();
