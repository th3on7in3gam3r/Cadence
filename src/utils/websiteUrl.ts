...
/** Fix common domain typos before crawl / partner API calls. */
...
export function looksLikeBrokenDomain(input: string): boolean {
  const host = domainFromBrandUrl(input);
  return host.endsWith('.om') && !host.endsWith('.com');
}
