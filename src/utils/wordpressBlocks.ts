...
import { plainHeadingTitle } from './markdownToc';
...
function headingBlock(level: number, text: string, slugger: GithubSlugger): string {
  const tag = `h${Math.min(6, Math.max(1, level))}`;
  const html = inlineMarkdownToHtml(text);
  const plain = plainHeadingTitle(text);
  const id = level >= 2 && plain ? slugger.slug(plain) : '';
  const idAttr = id ? ` id="${escapeHtml(id)}"` : '';
  return `<!-- wp:heading {"level":${level}} -->
<${tag} class="wp-block-heading"${idAttr}>${html}</${tag}>
<!-- /wp:heading -->`;
}
...
function buildSeoReferenceSection(seo: WordPressSeoMeta, slugger: GithubSlugger): string {
  const blocks: string[] = [
    separatorBlock(),
    headingBlock(3, 'SEO Optimization Notes', slugger),
    paragraphBlock(
  ...
  if (seo.seoInstructions?.trim()) {
    blocks.push(headingBlock(4, 'Keywords & search strategy', slugger));
...
export function toWordPressBlocks(markdown: string, seo?: WordPressSeoMeta): string {
  ...
  while (i < lines.length) {
    ...
    if (headingMatch) {
      blocks.push(headingBlock(headingMatch[1].length, headingMatch[2], slugger));
      i += 1;
      continue;
    }
  ...
  if (seo && hasSeoContent(seo)) {
    parts.push(buildSeoReferenceSection(seo, slugger));
  }
